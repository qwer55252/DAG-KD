#!/bin/bash
# C_E2 — GRP-KD + Speaker Adversarial (단독)
# Track A (large→base), latent에서 speaker info 제거 (GRL).
# vs C_E1 → adversarial 단독 효과.
# GPU: $CUDA_VISIBLE_DEVICES (default 2,3)
export WANDB_API_KEY="${WANDB_API_KEY:?Set WANDB_API_KEY before running}"

OUT=outputs/wav2vec/grp_kd_disen_v1/C_E2_speaker_adv
mkdir -p "$OUT"

PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-2,3} python train_wav2vec.py \
  --wandb_project DAG-KD-wav2vec \
  --wandb_run grp_kd_disen_v1_C_E2 \
  --out "$OUT" \
