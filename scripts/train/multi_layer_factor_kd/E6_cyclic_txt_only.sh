#!/bin/bash
# E6: Multi-Layer Cyclic Reconstruction (GRL, 3쌍) + txt KD only (generative)
# MI 항 전부 비활성 → cyclic reconstruction으로 대체
# spk KD 제거 (multi_kd_use_spk False) — txt 쪽만 distill

CUDA_VISIBLE_DEVICES=0 python train.py \
  --wandb_run multi_kd_E6_cyclic_txt_only \
  --out outputs/multi_layer_factor_kd/E6_cyclic_txt_only \
