...
class DisentanglementBottleneck(nn.Module):
    ...
    def _vclub_mi_upper_and_lll(
        ...
        for _ in range(num_neg):
            perm = torch.randperm(B, device=v.device) # TODO: 이렇게 하면 인덱스가 그대로인 것들이 있을 것 같은데?
            v_shuf = v[perm]
        ...
        # TODO: 여기서 joint와 marg를 각각 logging 할 수 있도록 수정 (joint는 크게 나와야 하고 marg는 작게 나와야 함.)
        mi_upper_raw = joint - marg
        lll = -joint

        return mi_upper_raw, lll
    ...
    def forward(
        ...
        if compute_loss:
            ...
            def add_pair(name, club, u_dyn, v_stat):
                nonlocal mi_enc_total, lll_club_total

                # (A) encoder용 MI: detach_for_lll=False (즉 u,v에 gradient 허용)
                mi_raw, _ = self._vclub_mi_upper_and_lll(
                ...
                _, lll = self._vclub_mi_upper_and_lll(
                    club, u_dyn, v_stat, enc_len,
